...
import de.upb.cs.brocoli.library.MessageBody
 ...
 */
class MessageChooser(private val kodein: Kodein, private val pipe: Pipe, private val ownID: UserID, private val neighborID: UserID, private val serviceIdWhitelist: List<ServiceId>? = null) {

    /**
     * Run a new thread to send Messages in Chat Repository and add a observer to listen to incoming messages.
     */
    fun run() {
        val messageRouter: MessageRouter = kodein.instance<MessageRouterFactory>().create(kodein, pipe, ownID, neighborID, serviceIdWhitelist)
        launch {
...
interface MessageRouterFactory {
    fun create(kodein: Kodein, pipe: Pipe, ownID: UserID, neighborID: UserID, serviceIdWhitelist: List<ServiceId>? = null): MessageRouter
}
