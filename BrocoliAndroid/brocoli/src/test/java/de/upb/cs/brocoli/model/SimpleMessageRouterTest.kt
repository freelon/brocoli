...
class SimpleMessageRouterTest {
    ...
    fun listExchangeMessagesTest() {
        val messageChooser = MessageChooser(kodein, pipe, ownID, neighborID)
        messageChooser.run()
        val listExchangeMessage = ListExchangeMessage("5", "1", listOf("2"))
        ...
        Thread.sleep(100)
        algorithmContentMessage = AlgorithmContentMessage("3", "2", "1", 1, 340472L, 1, BrocoliPriority.High, "Message_3".toByteArray().toTypedArray())
        pipeObserver.messageDeliveryResult(message = algorithmContentMessage, result = Pipe.DeliveryResult.Success)
        Thread.sleep(500)
        verify(mock = pipe, mode = times(3)).pushMessage(capture(messageCaptor)) //2 for send and 1 for List exchange Message
        ...
        val algorithmContentMessage2: AlgorithmContentMessage = messageList[2] as AlgorithmContentMessage
        val messageID2 = algorithmContentMessage2.id
        Assert.assertTrue(listMessage.knownMessageIds.containsAll(listOf("1", "2", "3")))
        Assert.assertTrue(messageID1 in listOf("1", "3"))
        Assert.assertTrue(messageID2 in listOf("1", "3"))
    ...
    fun receiveAlgorithmContentMessageTest() {
        ...
        val algorithmContentMessage = AlgorithmContentMessage("4", "2", "3", 1, 340472L, 1, BrocoliPriority.High, "Message_4".toByteArray().toTypedArray())
        pipeObserver.onMessageReceive(algorithmContentMessage)
        Assert.assertTrue(inMemoryRepository.getAllMessagesAsList().size == 4)
    }
}
